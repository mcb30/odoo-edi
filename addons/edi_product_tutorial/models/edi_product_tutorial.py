...
class EdiDocument(models.Model):
    ...
    product_tutorial_ids = fields.One2many(
        "edi.product.tutorial.record", "doc_id", string="Products"
    )
...
class EdiProductTutorialRecord(models.Model):
    ...
    weight = fields.Integer(string="Weight", required=True, readonly=True, help="Weight (in grams)")
    volume = fields.Integer(
        string="Volume", required=True, readonly=True, help="Volume (in cubic centimetres)"
    )
