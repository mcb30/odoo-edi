...
class Record(object):
    ...
    def __repr__(self):
        ...
        fields = sorted(
            (
                (k, getattr(self, k), getattr(cls, k).chars.start)
                for k in dir(cls)
                if (
                    k != "SDATA"
                    and isinstance(getattr(cls, k), Field)
...
class IDoc(object):
    ...
    def __init__(self, raw):
        ...
        self.data = [self.DataRecords[self.DataRecord(x).SEGNAM](x) for x in lines]

    def __repr__(self):
        """Generate printable representation"""
        return "%s(%r, %r)" % (self.__class__.__name__, self.control, self.data)
...
class Syntax(object):
    ...
    Field = Node(
        "Field",
        ["name", "text", "type", "length", "field_pos", "character_first", "character_last"],
    )
    ...
    Segment = Node(
        "Segment",
        ["name", "segmenttype", "qualified", "level", "status", "loopmin", "loopmax", "fields"],
    )

    SegmentGroup = Node(
        "SegmentGroup", ["number", "level", "status", "loopmin", "loopmax", "segments"]
    )
