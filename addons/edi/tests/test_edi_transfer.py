...
class TestEdiTransfer(EdiCase):
    ...
    def assertActionDomains(self, xfer):
        ...
        self.assertEqual(EdiDocument.search(view_docs['domain']),
                         xfer.doc_ids)

        # Check view input attachments
        view_inputs = xfer.action_view_inputs()
        self.assertEqual(IrAttachment.search(view_inputs['domain']),
                         xfer.input_ids)

        # Check view output attachments
        view_outputs = xfer.action_view_outputs()
        self.assertEqual(IrAttachment.search(view_outputs['domain']),
                         xfer.output_ids)
