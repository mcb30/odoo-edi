...
class DummySSHServer(paramiko.ServerInterface):
    ...
    def __init__(self, host_key, username=DEFAULT_USERNAME, password=DEFAULT_PASSWORD):
    ...
    def connect(self, ssh, *args, orig_connect=paramiko.SSHClient.connect, **kwargs):
    ...
    def create_transport(self, sock):
        ...
        transport.add_server_key(paramiko.RSAKey.from_private_key_file(str(self.host_key)))
...
class EdiGatewayCase(EdiCase):
    ...
    def setUp(self):
        ...
        patch_ssh_connect = patch.object(
            paramiko.SSHClient, "connect", autospec=True, side_effect=self.ssh_server.connect
        )
        patch_ssh_connect.start()
        self.addCleanup(patch_ssh_connect.stop)
        patch_ssh_close = patch.object(
            paramiko.SSHClient, "close", autospec=True, side_effect=self.ssh_server.close
        )
...
class EdiGatewayCommonCase(EdiGatewayCase):
    ...
    def test02_action_view_paths(self):
        ...
        self.assertEqual(len(EdiPath.search(action["domain"])), self.gateway.path_count)
    ...
    def test05_ssh_connect(self):
        ...
        with patch.object(edi_gateway._logger, "warning", autospec=True) as mock_log_warning:
            ssh = self.gateway.ssh_connect()
            ssh.close()
        self.assertTrue(mock_log_warning.called)
        self.assertEqual(
            self.gateway.ssh_host_fingerprint, "e3:32:6e:5c:ee:47:58:2d:bb:f1:d0:3b:0e:c4:55:a0"
        )
...
class EdiGatewayConnectionCase(EdiGatewayCase):
    ...
    def test07_safety_catch(self):
        ...
        EdiTransfer = self.env["edi.transfer"]
        self.gateway.path_ids.unlink()
        with patch.object(config, "get_misc", autospec=True) as mock_get_misc, patch.object(
            EdiTransfer.__class__, "do_transfer", autospec=True
        ) as mock_do_transfer:
    ...
    def test12_transfer_send_once(self):
        ...
        blocking_transfer = EdiTransfer.create(
            {"gateway_id": self.gateway.id, "output_ids": [(6, 0, attachment.ids)]}
        )
        with self.patch_paths({self.path_send: []}) as ctx:
            self.path_send.glob = "*.txt"
...
class EdiGatewayFileSystemCase(EdiGatewayConnectionCase):
    ...
    Context = namedtuple("EdiGatewayFileSystemCaseContext", ["temppath", "subpaths", "path_files"])
    ...
    def patch_paths(self, path_files):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            subpaths = {
                path: temppath.joinpath(subdir) for path, subdir in self.path_subdirs.items()
            }
    ...
    def assertSent(self, ctx, path_files):
        ...
        expected = {
            path.name: set(
                (file, self.files.joinpath(file).read_bytes())
                for file in (set(ctx.path_files.get(path, ())) | set(path_files.get(path, ())))
            )
            for path in ctx.subpaths
        }
        actual = {
            path.name: set((file.name, file.read_bytes()) for file in subpath.iterdir())
