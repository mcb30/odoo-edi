...
class TestAutocreate(EdiCase):
    ...
    def test_order(self):
        ...
        self.assertEqual(
            docs[0].input_ids.sorted("id").mapped("name"), ["dummy.txt", "hello_world.txt"]
        )
        self.assertEqual(docs[1].input_ids.sorted("id").mapped("name"), ["res.users.csv"])
        docs = self.autocreate("res.users.csv", "dummy.txt", "hello_world.txt")
        self.assertEqual(len(docs), 2)
        self.assertEqual(docs[0].input_ids.sorted("id").mapped("name"), ["res.users.csv"])
        self.assertEqual(
            docs[1].input_ids.sorted("id").mapped("name"), ["dummy.txt", "hello_world.txt"]
        )
