...
class Message(models.Model):
    ...
    edi_attachment_audit_ids = fields.One2many("edi.attachment.audit", "mail_message_id")
...
class EdiAttachmentAudit(models.Model):
    ...
    attachment_id = fields.Many2one(
        "ir.attachment", string="Attachment", index=True, readonly=True, ondelete="set null"
    )
    ...
    def audit_attachments(self, thread, attachments, **kwargs):
        ...
        if attachments:
            ...
            thread.message_post(
                attachment_ids=[x.id for x in attachments],
