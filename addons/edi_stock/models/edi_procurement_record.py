...
class EdiProcurementRecord(models.Model):
    ...
    route_key = fields.Char(
        string="Route Key", required=True, readonly=True, index=True, edi_relates="route_id.name"
    )
    route_id = fields.Many2one(
        "stock.location.route", string="Route", required=False, readonly=True, index=True
    )
