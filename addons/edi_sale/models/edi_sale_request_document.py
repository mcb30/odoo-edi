"""EDI sale request documents"""

from odoo import api, models


class EdiSaleRequestDocument(models.AbstractModel):
    """EDI sale request document

    This is the base model for EDI sale request documents.
    Each row represents two collections of EDI records:

    - a collection of EDI sale request records that, in
      turn, each represent a sale that will be created
      when the document is executed, and

    - a collection of EDI sale line request records that, in turn,
      each represent a line item within one of the above sale orders
    ...
    @api.model
    def sale_line_request_record_model(self, doc,
                                       supermodel='edi.sale.line.request.record'):
