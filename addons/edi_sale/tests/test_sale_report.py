...
class TestSaleReport(EdiSaleCase):
    ...
    def setUpClass(cls):
        ...
        cls.rec_type_sale_report = cls.env.ref("edi_sale.sale_report_record_type")
        cls.rec_type_sale_line_report = cls.env.ref("edi_sale.sale_line_report_record_type")
        cls.doc_type_sale_report = EdiDocumentType.create(
            {
                ...
                "rec_type_ids": [
                    (6, 0, [cls.rec_type_sale_report.id, cls.rec_type_sale_line_report.id])
                ],
            }
        )
    ...
    def test_dummy(self):
        ...
        self.assertEqual(line_reports.mapped("line_ids"), self.sale.order_line)
        line_reports_by_product = {x.product_id.default_code: x for x in line_reports}
