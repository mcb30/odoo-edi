...
class EdiDocument(models.Model):
    ...
    location_tutorial_ids = fields.One2many(
        "edi.location.tutorial.record", "doc_id", string="Locations"
    )
...
class EdiLocationTutorialRecord(models.Model):
    ...
    parent_key = fields.Char(
        string="Parent Key", required=False, readonly=True, edi_relates="parent_id.barcode"
    )
    parent_id = fields.Many2one(
        "stock.location", string="Parent Location", required=False, readonly=True, index=True
    )
    shelf = fields.Integer(string="Shelf", required=True, readonly=True, help="Shelf number")
