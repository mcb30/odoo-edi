...
class EdiDocument(models.Model):
    ...
    @api.depends("pick_request_tutorial_ids", "pick_request_tutorial_ids.pick_id")
    def _compute_pick_ids(self):
        super()._compute_pick_ids()
        self.mapped("pick_request_tutorial_ids.pick_id")
        for doc in self:
...
class EdiMoveRequestTutorialRecord(models.Model):
    ...
    action = fields.Selection(
        ...
        readonly=True,
        index=True,
        selection=[("C", "Create"), ("U", "Update"), ("D", "Delete")],
    )
...
class EdiPickRequestTutorialDocument(models.AbstractModel):
    ...
    def pick_types_map(self, doc):
        ...
        prefix_test = lambda prefix: re.compile(
            r"%s[\W\d_]" % next(x for x in reversed(prefix.split("/")) if x), flags=re.IGNORECASE
        )
        return [
            (prefix_test(x.sequence_id.prefix), x)
            for x in doc.doc_type_id.pick_type_ids
            if x.sequence_id.prefix
        ]
    ...
    def prepare(self, doc):
        ...
        for fname, data in doc.inputs():
            ...
            pick_type = PickingType.union(*(v for k, v in pick_type_map if k.match(fname)))
            if not pick_type:
                raise UserError(_('"%s" matches no picking types') % fname)
            if len(pick_type) > 1:
                raise UserError(
                    _('"%s" matches multiple picking types: %s')
